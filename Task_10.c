...
int main() {
 ...
 fwrite(&orderCount, sizeof(int), 1, file); // Записываем количество
заказов в файл
 for (int i = 0; i < orderCount; ++i) {
 fwrite(&orders[i], sizeof(Order), 1, file); // Записываем информацию
о заказе в файл
 }
 fclose(file);
...
orderCount);
 ...
 products = updateProducts(products, &productCount, readOrdersData, 
orderCount);
